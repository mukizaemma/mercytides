...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (Schema::hasColumn('settings', 'primary_color')) {
            ...
            if ($current === '' || in_array($current, ['#fad200', '#ffe135', '#ffeb3b', '#ffc107'], true)) {
                $updates['primary_color'] = '#3386B5';
            }
        }

        if (Schema::hasColumn('settings', 'secondary_color')) {
            $current = strtolower((string) ($row->secondary_color ?? ''));
            if ($current === '' || in_array($current, ['#2c2c2c', '#1a1a1a', '#333333', '#2e7d32'], true)) {
                $updates['secondary_color'] = '#00205B';
            }
        }

        if (Schema::hasColumn('settings', 'neutral_color')) {
            $current = strtolower((string) ($row->neutral_color ?? ''));
            if ($current === '' || in_array($current, ['#b0b0b0', '#03a9f4', '#0288d1'], true)) {
                $updates['neutral_color'] = '#58A9C9';
            }
        }
