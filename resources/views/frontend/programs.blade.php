            ...
            <div class="row g-4">
                ...
                data-wow-delay=".3s">
                    <article class="program-list-card h-100 d-flex flex-column bg-white rounded-3 overflow-hidden border shadow-sm">
                        <a href="{{ route('programShow', ['slug' => $rs->slug]) }}" class="program-list-card__thumb d-block position-relative">
                            <img src="{{ $rs->coverImageUrl() ?: asset('assets/img/blog/blog-bg-1.jpg') }}" alt="{{ $rs->title }}" class="w-100 program-list-card__img">
                        </a>
                        <div class="program-list-card__body p-4 d-flex flex-column flex-grow-1">
                            <h2 class="h5 mb-3">
                                <a href="{{ route('programShow', ['slug' => $rs->slug]) }}" class="text-dark text-decoration-none">{{ $rs->title }}</a>
                            </h2>
                            <div class="tp-about-3__text flex-grow-1">
                                @php
                                $descPlain = strip_tags(html_entity_decode($rs->description ?? ''));
                                $words = \Illuminate\Support\Str::limit($descPlain, 140, '…');
                                @endphp

                                <p class="program-list-card__excerpt mb-4">{{ $words }}</p>

                                <a href="{{ route('programShow', ['slug' => $rs->slug]) }}" class="tp-btn program-list-card__cta align-self-start mt-auto">
